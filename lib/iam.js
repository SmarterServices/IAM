(function(){
    ...
    function createRegExPatternForResource (input)
    {
        ...
        return ('^' + new_parts.join('') + '$').replace(/[^\x00-\x7F]/g, "");
    ...
    function authorize( resource, action, data ) {
        ...
        if(Array.isArray(resource)) {
            defaultDeny = resource.every(res => {
                let localDDeny = false;
                data.Statement.forEach(stm => {
            ...
            data.Statement.forEach(stm => {
                defaultDeny = processBlock(stm.Effect === 'Allow',findActionForResource( resource,  action, [stm] ),defaultDeny)
            })
        }
    ...
    function findActionForResource( resource, action, stmts ) {
        for (var i in stmts) {
            ...
            for ( var j in stmt.ResourceRegex ) {
                ...
                let localStatment = stmt.ResourceRegex[j].split(':').map(x => x === "" ? "([0-9A-Za-z\-]+)*" : x).join(":")
                var regex = new RegExp(localStatment);
                var resource_parts = resource.split(':');
                var regex_resource_parts = stmt.Resource[0].split(':');
                var resource = resource.split(':').map(x => x === "" ? "PREFILL" : x).join(":")
