...
describe('During authorization checks', function(){
    ...
    describe('should successfully to grant permission', function(){
        ...
        it('with a wildcard authorization resource', function(){
            ...
            expect(iam.authorize('ssrn:ss:sm:::search', 'sm:CanRead',processedIam)).to.equal(false);
...
describe('During gathering the action criteria', function(){
    ...
    it('should allow mix of wildcards', function(){
        var sampleIam = {
            "Statement": [
                {
                    "Effect": "Allow",
                    "Action": ["*"],
                    "Resource": "ssrn:ss:iam::*"

                },
                {
                    "Effect": "Deny",
                    "Action": ["*"],
                    "Resource": ["ssrn:ss:iam::exam/*","ssrn:ss:iam::course/*"]

                },
                {
                    "Effect": "Allow",
                    "Action": ["*"],
                    "Resource": ["ssrn:ss:iam::exam/100","ssrn:ss:iam::course/102"]
        ...
        expect(iam.authorize('ssrn:ss:iam::course/102', 'CanRead',processedIam)).to.equal(true);
        expect(iam.authorize('ssrn:ss:iam::exam/100', 'CanRead',processedIam)).to.equal(true);
        expect(iam.authorize('ssrn:ss:iam::course/101', 'CanRead',processedIam)).to.equal(false);
        expect(iam.authorize('ssrn:ss:iam::exam/101', 'CanRead',processedIam)).to.equal(false);
        expect(iam.authorize('ssrn:ss:iam::offering/101', 'CanRead',processedIam)).to.equal(true);
    });
    it('should allow mix of wildcards and array in Resource', function(){
        var sampleIam = {
            "Statement": [
                {
                    "Effect": "Allow",
        ...
        expect(iam.authorize(['ssrn:ss:iam::course/102','ssrn:ss:iam::exam/100'], 'CanRead',processedIam)).to.equal(true);
        expect(iam.authorize(['ssrn:ss:iam::course/101','ssrn:ss:iam::exam/100'], 'CanRead',processedIam)).to.equal(false);
        expect(iam.authorize(['ssrn:ss:iam::course/102'], 'CanRead',processedIam)).to.equal(true);
        expect(iam.authorize(['ssrn:ss:iam::course/99'], 'CanRead',processedIam)).to.equal(false);
        expect(iam.authorize(['ssrn:ss:iam::course/102','ssrn:ss:iam::exam/100','ssrn:ss:iam::offering/101'], 'CanRead',processedIam)).to.equal(true);
        expect(iam.authorize(['ssrn:ss:iam::course/102','ssrn:ss:iam::exam/60','ssrn:ss:iam::offering/101'], 'CanRead',processedIam)).to.equal(false);
        expect(iam.authorize(['ssrn:ss:iam::offering/101'], 'CanRead',processedIam)).to.equal(true);
        expect(iam.authorize(['ssrn:ss:iam::course/99'], 'CanRead',processedIam)).to.equal(false);
    });
    it('realistic statement', function(){
        var sampleIam = {
            "Statement": [
         {
             "Effect": "Allow",
         ...
         },
         {
            ...
            "Resource": [
                "ssrn:ss:sp::1000:one/*",
                "ssrn:ss:sp::1000:two/*",
                "ssrn:ss:sp::1000:three/*",
                "ssrn:ss:sp::1000:four/*",
                "ssrn:ss:sp::1000:five/*",
                "ssrn:ss:sp::1000:six/*"
                ]
         },
         {
            ...
            "Action":"*",
            "Resource": [
                "ssrn:ss:sp::1000:one/100",
                "ssrn:ss:sp::1000:two/200",
                "ssrn:ss:sp::1000:three/300",
                "ssrn:ss:sp::1000:four/400",
                "ssrn:ss:sp::1000:five/500",
                "ssrn:ss:sp::1000:six/600"
                ]
         }
        ...
        expect(iam.authorize(['ssrn:ss:sp::1000:blah/2134'], 'sp:CanRead',processedIam)).to.equal(true);
        expect(iam.authorize(['ssrn:ss:sp::1000:blah/2134'], 'sn:CanRead',processedIam)).to.equal(false);
        expect(iam.authorize(['ssrn:ss:sp::1000:blah/2134'], 'sp:blahRead',processedIam)).to.equal(false);
        expect(iam.authorize(['ssrn:ss:sp::1000:one/2134'], 'sp:CanRead',processedIam)).to.equal(false);
        expect(iam.authorize(['ssrn:ss:sp::1000:one/100'], 'sp:CanRead',processedIam)).to.equal(true);
        expect(iam.authorize(['ssrn:ss:sp::1000:blah/2134','ssrn:ss:sp::1000:one/2134'], 'sp:CanRead',processedIam)).to.equal(false);
        expect(iam.authorize(['ssrn:ss:sp::1000:'], 'sp:CanRead',processedIam)).to.equal(true);
    });
